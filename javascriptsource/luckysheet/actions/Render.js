 ...
 */
export async function Render(containerId, sheetTitle, collaborate, url) {
	// BEGIN USER CODE
			...
			switch (event.data.mx_luckysheet.id) {
				case 'load':
					...
					sendCommand({
						mx_luckysheet: {
							id: 'setup',
							data: file ? file : [emptySheet],
							updateUrl: collaborate ? `${mx.appUrl}ws`.replaceAll('http://', 'ws://').replaceAll('https://', 'wss://') : undefined, lang: mx.session.sessionData.locale.code.split('_')[0]
						}
					});

					//wait setup（setup完成初始化表格前，load触发的update会报错）
					//FIXME 临时方案，不稳定
					setTimeout(() => {
					...
					break;
				case 'cellUpdated':
					on.emit(container, 'cellUpdated', { bubbles: true, data });
					break;
				case 'rangeSelect':
					on.emit(container, 'rangeSelect', { bubbles: true, data });
					break;
				case 'rangePasteAfter':
					on.emit(container, 'rangePasteAfter', { bubbles: true, data });
					break;
				case 'rangePasteBefore':
					on.emit(container, 'rangePasteBefore', { bubbles: true, data });
					break;
				case 'rangeMoveAfter':
					on.emit(container, 'rangeMoveAfter', { bubbles: true, data });
				case 'updated':
					on.emit(container, 'updated', { bubbles: true, data });
					break;
			}
